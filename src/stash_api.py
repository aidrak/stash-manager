...
class StashAPI:
    ...
    def _get_stashdb_scenes_paginated(self, limit: int = None, start_date: str = None, end_date: str = None) -> List[Dict]:
        ...
        while len(all_scenes) < max_scenes:
            ...
            if start_date and end_date:
                logger.info(f"Setting date range filter: {start_date} to {end_date} (inclusive) - Page {page}")
            ...
            try:
                logger.info(f"Executing StashDB scene query page {page} with variables: {variables}")
                result = self.execute_query(query, variables)
                ...
                if "data" in result and "queryScenes" in result["data"]:
                    ...
                    if "scenes" in query_result:
                        ...
                        logger.info(f"Page {page}: Retrieved {len(page_scenes)} scenes (Total available: {total_count})")
        ...
        logger.info(f"Retrieved total of {len(all_scenes)} scenes from StashDB before date filtering")
        
        # Debug logging to see what dates we actually got
        logger.info(f"Sample scene dates from StashDB:")
        ...
        if start_date and end_date:
            ...
            try:
                ...
                logger.info(f"Filtered to {len(filtered_scenes)} scenes within date range {start_date} to {end_date}")
    ...
    def _get_local_stash_scenes(self, limit: int = None, start_date: str = None, end_date: str = None) -> List[Dict]:
