...
class DatabaseManager:
    ...
    def add_filter_rule(self, context: str, name: str, field: str, operator: str, 
                       value: str, action: str) -> int:
        """Add a new filter rule."""
        with self.get_connection() as conn:
            # Get next priority
            cursor = conn.execute(
                "SELECT COALESCE(MAX(priority), 0) + 1 FROM filter_rules WHERE context = ?",
                (context,)
            )
    ...
    def update_filter_rule(self, rule_id: int, field: str, operator: str, 
                          value: str, action: str):
    ...
    def delete_filter_rule(self, rule_id: int):
        ...
        with self.get_connection() as conn:
            ...
            cursor = conn.execute(
                "SELECT context, priority FROM filter_rules WHERE id = ?",
                (rule_id,)
            )
            row = cursor.fetchone()
            if not row:
    ...
    def finish_job_run(self, job_run_id: int, status: str, scenes_processed: int = 0,
                      scenes_added: int = 0, scenes_deleted: int = 0, 
        ...
        with self.get_connection() as conn:
            conn.execute(
                """UPDATE job_runs 
                   ...
                   WHERE id = ?""",
                (status, scenes_processed, scenes_added, scenes_deleted, error_message, job_run_id)
            )
            conn.commit()
