...
class SceneFilter:
    ...
    def __init__(self, config: Dict):
        ...
        logger.info(f"Cup size exceptions: {len(self.exceptions_to_large)} large, {len(self.force_to_small)} small")
        logger.info(f"Keyword filter enabled: {self.keyword_enabled} with {len(self.keywords)} keywords: {self.keywords}")
    ...
    def should_remove_scene(self, scene_data: Dict) -> Tuple[bool, str]:
        ...
        if self.ethnicity_enabled:
            for performer in performers:
                ...
                if ethnicity and any(value.lower() in ethnicity.lower() for value in self.ethnicity_values):
