...
class RuleSyncManager:
    ...
    def are_rules_in_sync(self) -> Tuple[bool, str]:
        ...
        if len(add_rules) != len(clean_rules):
            return False, f"Rule count mismatch: {len(add_rules)} add rules vs {len(clean_rules)} clean rules"
        ...
        for i, add_rule in enumerate(add_rules):
            ...
            if (converted_rule['field'] != clean_rule['field'] or
                converted_rule.get('match', converted_rule.get('operator')) != clean_rule.get('match', clean_rule.get('operator')) or
                converted_rule.get('value') != clean_rule.get('value') or
                converted_rule.get('action') != clean_rule.get('action')):
                
                return False, f"Rule {i+1} mismatch: Add rule converts to different values than Clean rule"
    ...
    def update_sync_settings(self, enabled: bool, direction: str):
        ...
        if enabled and not self.sync_enabled:
            ...
            if not in_sync:
                raise ValueError(f"Rules must be in sync before enabling Rule Synchronization. {reason}")
    ...
    def sync_rules(self, source_context: str, rules: List[Dict]):
        ...
        if not self.should_sync_rule(source_context):
            logger.debug(f"Rule sync disabled or wrong direction for {source_context}")
            return
